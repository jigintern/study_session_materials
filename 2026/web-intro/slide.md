...
| `<html lang="ja">` | HTMLの始まり。`lang="ja"` は日本語 |
| `<head>`, `<meta>` | ページの設定|
| `<link>` | CSSファイルを読み込む |
| `<script src="script.js">` | JSファイルを読み込む（応用課題で使う） |
| `<body>` | **ページの中身**（ここに書いたものが画面に表示される） |
...
HTMLは **タグ** で書きます。タグには **開始タグ** と **終了タグ** があります。
...
`index.html` の `<body>` 直下（container の前）に追加します。
...
| `border-radius: 0 0 20px 20px` | 角を丸くする（下で詳しく説明） |
...
`#` の後に **赤(R)・緑(G)・青(B)** を16進数で2桁ずつ指定する。
より細かい色が指定できる。
...
| コード | 赤 | 緑 | 青 | 色 |
|--------|-----|-----|-----|-----|
| `#ff0000` | ff(最大) | 00 | 00 | 赤 |
| `#0000ff` | 00 | 00 | ff(最大) | 青 |
| `#000000` | 00 | 00 | 00 | 黒 |
| `#ffffff` | ff | ff | ff | 白 |

> カラーコードは覚えなくてOK。ネットで「カラーピッカー」と検索すると選べます。
> `#fff` は `#ffffff` の省略形です。
...
`index.html` の `<div class="container">` の中に追加：
...
![bg right](imgs/adv-e-slideshow.gif)
