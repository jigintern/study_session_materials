...
# Chapter 1: テンプレートを確認しよう

テンプレートを開くと、HTML にクイズの骨組みがすでに入っています

プレビューを見ると文字やボタンは表示されていますが…
**まだ見た目がさみしい！** 😢
...
CSSの `/* 全体のレイアウト */` セクションの空欄に値を入力しよう
...
| `background` | `linear-gradient(135deg, #667eea, #764ba2)` | 紫のグラデーション背景 |

<div class="check">

✅ 背景が紫のグラデーションになり、白い箱が画面中央に表示されましたか？
...
| プロパティ | 入力する値 | 意味 |
|-----------|-----------|------|
| `background` | `#f0f0ff` | ホバー時の薄い色 |
| `transform` | `translateY(-2px)` | 少し浮き上がる |

<div class="check">

✅ ボタンにマウスを乗せると色が変わって浮き上がりますか？
...
**正解ボタン** `.choice-btn.correct` → `background: #4caf50`, `color: white`, `border-color: #4caf50`

**不正解ボタン** `.choice-btn.wrong` → `background: #e53935`, `color: white`, `border-color: #e53935`

**「次の問題へ」** `#next-btn` → `background: #667eea`, `color: white`, `border-radius: 10px`

**ホバー** `#next-btn:hover` → `background: #5a6fd6`

<div class="check">

✅ きれいなクイズ画面が完成しましたか？ まだボタンは動きません — 次の Chapter へ！
