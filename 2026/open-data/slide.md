...
`ワークスペース設定を開く(JSON)` を選ぶ（日本語 UI では「基本設定: ワークスペース設定を開く (JSON)」）
...
`map.html` なども同梱されている場合は一緒にコピーしてください（詳細は `template/README.md`）。

---

## Chart.js とは

**Web で最も広く使われるグラフライブラリのひとつ**
...
- ブラウザは **`file://` で直接開かず**、Live Server など **`http://localhost` で開く**（地図ページと同様、`file://` だと挙動が不安定になりやすい）
