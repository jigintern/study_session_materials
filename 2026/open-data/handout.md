...
講義の進行に合わせた **正解参照** は [`CODE_REFERENCE.md`](./CODE_REFERENCE.md) にまとまっています。  
...
[`template/`](./template/) をコピーしたフォルダで作業します。テンプレートの **`app.js` は Chart.js より前の状態**（コメントのみ）です。`index.html` では Chart.js を先に読み込んでから `app.js` を読み込みます。
