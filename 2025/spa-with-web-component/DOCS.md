...
開いたファイルに以下の[コード 3.1.2-1.](#c321-1)の内容を貼り付けてください。

<figure>

<figcaption><a id="c312-1">コード 3.1.2-1. ユーザー設定(JSON)</a></figcaption>
...
ここには「HTML」というウェブページの構造を表現する言語が表示されています。また、タブの下部には「CSS」というウェブページの要素の表示を変更する言語が表示されています。
...
<figcaption><a id="c52-1">コード 5.2-1. DOCTYPE宣言を追加したindex.html</a></figcaption>
...
以下の[コード 5.3.4-1.](#c534-1)に示すHTMLを、コメントを置き換えつつ`index.html`に追記してください。
...
普段私達が目にするウェブサイトは画像を利用しているところがほとんどでしょう。HTMLでは`<img>`要素を使用して画像をページに表示できます。
...
そのため、現在ウェブ開発では **全称セレクタ**`*` を用いて以下のように設定するのが一般的です。
...
この節では、以下に示す[コード 6.4.1-1.](#c641-1)のHTMLを改変しながらCSSを用いて装飾し、聖杯レイアウトを実装することを考えます。  
...
JavaScriptとは、一般にウェブ開発やシステム開発に用いられるプログラミング言語の一つです。
...
そのため、変数・関数には、処理の内容や意味、効果が一目でわかるような、簡潔でわかりやすい命名をするべきだとされています。  
...
特にクラスのメソッドとして、ある式を解くような関数の場合では悪い例は悪いとは言い切れません。  
また、エディタの補完機能がない場合には冗長な命名の関数は嫌われるため、良い例として示したものが必ずしも良いわけではありません。

時と場合に合わせて命名とコメントを組み合わせながら、誰が読んでもわかりやすいコードになるよう心がけましょう。
...
論理演算はプログラムで頻出する演算の一つです。  
それぞれの演算と記述は以下の[表 7.2.5-1](#t725-1)のように対応します。
...
if文は`()`内の評価結果が真のときに直後の`{}`で囲まれたブロック内の処理を行い、`()`内の評価結果が偽のときに直後の`else`句の処理を行います。このとき、`else`のあとに`if`をつなげることで条件分岐を追加できます。`else`句のあとにある`{}`ブロック内の処理は、それより前にある`if`と`else if`のすべての条件を満たさなかったときに実行されます。
...
配列には`.forEach()`というメソッドがあります。メソッドとはオブジェクトから呼び出せる関数のことで、メソッドや関数については[8. JavaScript（Ⅱ）](#8-javascript-ⅱ)で詳しく説明します。
...
[8. JavaScript（Ⅱ）](#8-javascript-ⅱ)では、この手間を小さくできる「処理に名前をつけて呼び出す仕組み」をいくつか説明します。
...
TODO一覧画面とTODO編集画面の2つの画面からなるウェブアプリケーションで、データの保存には[`localStorage`](https://developer.mozilla.org/ja/docs/Web/API/Window/localStorage)を利用します。
...
開発をスムーズにするための、わかりやすいディレクトリ構成について説明します。  
...
<figcaption><a id="c111-1">コード c11.1-1. ディレクトリ構成</a></figcaption>
...
`shared`は各JSファイル中での記述を共通化したファイルを配置するディレクトリです。

`src`以下に配置されるファイルは、index.htmlやmain.mjsから読み込んで利用されます。  
以下の図 11.1-1.にソースファイルの利用関係のイメージ図を示します。

<figure>

<figcaption><a id="f111-1">図11.1-1. ソースファイルの利用関係のイメージ図</a></figcaption>

```mermaid
graph LR;
    A["index.html"] -->|実行| B["main.mjs"]
    B -->|利用| src
    C1["src/pages/"] -->|利用| D["src/components/"]
    D -->|利用| E["src/shared/"]
    C1 -->|利用| E
...
<figcaption><a id="f112-1">図 11.2-1. ルーティング処理からログ出力がされている</a></figcaption>
...
ブラウザで以下の[図 11.6.1-1.](#f1161-1)の画面の表示を確認できれば問題ありません。

<figure>

<figcaption><a id="f1161-1">図 11.6.1-1. カスタム要素の登録確認</a></figcaption>
...
ページ用カスタム要素と同じように、TODOアイテムもカスタム要素の作成と登録までを行います。  
...
しかし、それらのフレームワークも大元は標準機能であるウェブコンポーネントやDOM操作を使用して実装されています。このため、今回の実験で学習した内容はフレームワークの動作原理を理解するうえで少なからず役に立つものとなるでしょう。

この経験を活かして、受講いただいた皆様がウェブアプリケーション開発に少しでも興味を持っていただければ幸いです。

**📝課題 12-1.** [11. ウェブコンポーネントを利用したSPAの開発](#11-ウェブコンポーネントを利用したspaの開発)で作成したウェブアプリケーションを、[GitHub Pages](https://docs.github.com/ja/pages/getting-started-with-github-pages/what-is-github-pages)等を利用して公開しなさい。公開したウェブアプリケーションにアクセス可能なURLをレポートに添付しなさい。もし独自にカスタマイズした点があれば、合わせて説明を記述しなさい。（5点 + カスタマイズの個数1つにつき1点加点）
